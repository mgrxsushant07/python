...
def calculate_gpa():
    try:
        ...
        student_name = entry_name.get()  # Get student name
        if not student_name:
        ...
        # Save the GPA to the student records file
        with open("student_records.txt", "a") as file:
            file.write(f"Student Name: {student_name}, GPA: {gpa:.2f}\n")

        # Display success message for saving GPA
        messagebox.showinfo("Success", f"GPA ({gpa:.2f}) saved successfully for {student_name}.")
...
def save_record():
    try:
        ...
        with open("student_records.txt", "a") as file:
            file.write(f"Student Name: {student_name}, Mathematics: {math_marks}, Science: {science_marks}, {optional_subject}: {optional_marks}, Nepali: {nepali_marks}, English: {english_marks}, Social: {social_marks}, Computer: {computer_marks}\n")
...
# Student Name Entry
label_name = tk.Label(window, text="Student Name:", font=font, bg="#e0f7fa")
label_name.grid(row=0, column=0, padx=10, pady=5, sticky="w")
entry_name = tk.Entry(window, font=font)
entry_name.grid(row=0, column=1, padx=10, pady=5)

label_math = tk.Label(window, text="Mathematics:", font=font, bg="#e0f7fa")
label_math.grid(row=1, column=0, padx=10, pady=5, sticky="w")
...
label_science.grid(row=2, column=0, padx=10, pady=5, sticky="w")
...
label_optional.grid(row=3, column=0, padx=10, pady=5, sticky="w")
...
entry_optional.grid(row=3, column=1, padx=10, pady=5)
...
# Label to display GPA
result_label = tk.Label(window, text="", font=("Helvetica", 14, "bold"), bg="#e0f7fa")
